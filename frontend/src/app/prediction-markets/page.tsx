...
export default function PredictionMarketsPage() {
  const { data: rawPoly, loading, error, retry, refreshIn } = useApiData<any[]>(
    () => api.getPolymarketLeaderboard().then((poly) => (Array.isArray(poly) ? poly : [])),
    { refreshInterval: 120 }
  );
  const polyTraders = rawPoly ?? [];
