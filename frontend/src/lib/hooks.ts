...
const WATCHLIST_KEY = "smartflow_watchlist";
...
export function useWatchlist() {
  ...
  const add = useCallback(
    (type: WatchlistItem["type"], value: string) => {
      if (items.some((i) => i.type === type && i.value === value)) return;
      const newItems = [
        ...items,
        { type, value, addedAt: new Date().toISOString() },
      ];
      save(newItems);
    },
    [items]
  ...
  const isWatched = useCallback(
    (type: WatchlistItem["type"], value: string) => {
      return items.some((i) => i.type === type && i.value === value);
    },
    [items]
  );

  const politicians = items.filter((i) => i.type === "politician");
  const tickers = items.filter((i) => i.type === "ticker");

  return { items, politicians, tickers, add, remove, isWatched };
}
